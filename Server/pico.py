...
class Pico:
    ...
    class _Heater: #little mini-class as a helper
        def __init__(self,pico):
            ...
            self.pico=pico
        def update(self, reduced): #hysteresis-based control approach (easily swappable, of course)
            self.current=self.pico.temp-HC_S_OFFSET*reduced #don't bother updating, it should already be updated by now
...
def _control_loop(pico): #use the pico state to update the pico state on a loop
    while 1:
        ...
        pico.heater.update(pico.occ_guess) #apply control loop logic
